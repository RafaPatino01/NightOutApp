...
if (currentUrl.includes("edit_usuario")) {
    ...
    if (pId) {
        ...
        const apiUrl = `http://192.168.1.72:3000/get_usuarios/?correo_electronico=${pId.toLowerCase()}`;
...
if (pId) {
    // Construct the URL with the ID as a query parameter
    const apiUrl = `http://192.168.1.72:3000/get_establecimiento/${pId}`;
    ...
    .then(data => {
        ...
        if (establecimientoData) {
            ...
            const mapa_fullImageUrl = `http://192.168.1.72:3000${mapaUrl.substring(1)}`;
            ...
            for (const imageUrl of imageUrls) {
                ...
                const fullImageUrl = `http://192.168.1.72:3000${imageUrl.substring(1)}`;
...
function updateUser(userId) {
    ...
    fetch(`http://192.168.1.72:3000/update_usuario/${userId}`, {
        method: 'PUT',
        headers: {
...
function updateEstablecimiento(establecimientoId) {
    ...
    fetch(`http://192.168.1.72:3000/update_establecimiento/${establecimientoId}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
        },
